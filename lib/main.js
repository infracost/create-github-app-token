...
export async function main(
  ...
  // Validate the token works before exposing it
  core.info("Validating token...");
  await validateToken(request, authentication.token, core);
  core.info("Token is valid.");

  // Validate that each explicitly requested repository is accessible
  if (parsedRepositoryNames.length > 0) {
    core.info("Validating repository access...");
    await validateRepositoryAccess(request, authentication.token, parsedOwner, parsedRepositoryNames, core);
...
async function validateToken(request, token, core) {
  await pRetry(
    async () => {
      await request("GET /installation/repositories", {
        per_page: 1,
...
async function validateRepositoryAccess(request, token, owner, repositories, core) {
  for (const repo of repositories) {
    await pRetry(
      ...
      },
      {
        ...
        onFailedAttempt: (error) => {
          core.info(
            `Repository access validation failed for ${owner}/${repo} (attempt ${error.attemptNumber}): ${error.message}`
          );
        },
      }
    );
